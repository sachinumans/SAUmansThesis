...
Popt = ga(@(p)runModelSA(p, p_bio, k, xMeas, gaitCycle, bound, LgrfPos, RgrfPos, LgrfVec, RgrfVec, LgrfMag, RgrfMag, LLML, LGTR, RLML, RGTR, dt, false),...
            14,[],[],[],[],...
            [lb_vpp, lb_vpp, lb_spring, -0.5, lb_vpp, lb_spring, lb_J],...
            [ub_vpp, ub_vpp, ub_spring,  0.5, ub_vpp, ub_spring, ub_J], [],[],...
            optimoptions('ga','MaxTime', 2*60));

%%
runModelSA(Popt, p_bio, k, xMeas, gaitCycle, bound, LgrfPos, RgrfPos, LgrfVec, RgrfVec, LgrfMag, RgrfMag, LLML, LGTR, RLML, RGTR, dt, true);

%%
function [ResNorm] = runModelSA(p, p_bio, k, xMeas, gaitCycle, bound, LgrfPos, RgrfPos, LgrfVec, RgrfVec, LgrfMag, RgrfMag, LLML, LGTR, RLML, RGTR, dt, plotIO)
...
while ki < k(end)
    switch gaitCycle(1)
        ...
        case "lDSr"
            ...
            k_switch = [k_switch k_end];
            for ki = ki:k_end
                xModel(:,ki+1) = xModel(:,ki) + dt*lDSr_split_eom(0,xModel(:,ki)',LgrfPos(ki,1:2),RgrfPos(ki,1:2),Vl_ds,Vs_bl,Vs_fl,h,Wi,l0,m,K_ds,b_ds,J);
                
                p{9} = K_ds; p{10} = b_ds;
                p{11} = [Vs_bl, Vs_fl]; p{12} = Vl_ds; 
                p{13} = [[LgrfPos(ki,1:2)'; 0],[RgrfPos(ki,1:2)'; 0]]; p{14} = -1;
        ...
        case "rDSl"
            ...
            k_switch = [k_switch k_end];
            for ki = ki:k_end
                xModel(:,ki+1) = xModel(:,ki) + dt*rDSl_split_eom(0,xModel(:,ki)',LgrfPos(ki,1:2),RgrfPos(ki,1:2),Vl_ds,Vs_bl,Vs_fl,h,Wi,l0,m,K_ds,b_ds,J);

                p{9} = K_ds; p{10} = b_ds;
                p{11} = [Vs_bl, Vs_fl]; p{12} = Vl_ds; 
                p{13} = [[LgrfPos(ki,1:2)'; 0],[RgrfPos(ki,1:2)'; 0]]; p{14} = -2;
                grfModel(:,:,ki) = state2grf_DSsplit(xModel(:,ki), p);
...
xModelRes = blkdiag(diag([1,1,5]), zeros(3), eye(4)*10, zeros(4))*(xModel - xMeas)*diag(1:length(xModel));
xModelRes(isnan(xModelRes)) = 1e4;
xModelResNorm = norm(xModelRes, "fro");
...
ResNorm = xModelResNorm*50 + angResNorm*1 + magResNorm*1e-4 + legLenResNorm*10;
