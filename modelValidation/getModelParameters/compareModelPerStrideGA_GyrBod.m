function [ResNorm] = compareModelPerStrideGA_GyrBod(p, p_bio, p_spring, w, k, xMeas, walkVel, gaitCycle, bound, LgrfPos, RgrfPos, LgrfVec, RgrfVec, LgrfMag, RgrfMag, LLML, LGTR, RLML, RGTR, dt, plotIO)
    ...
    switch gaitCycle(1)
        case "lSS"
            ...
            for ki = ki:k_end
                ...
                xModel(:,ki+1) = xModel(:,ki) + dt*LSSeom_gyrBod(0,xModel(:,ki)',lFcur,Vl_ss,Vs_ss,h,Wi,l0+l_preload,m,K_ss,b_ss,gamx,gamy,rx,ry,alpha);
        ...
        case "rSS"
            ...
            for ki = ki:k_end
                ...
                xModel(:,ki+1) = xModel(:,ki) + dt*RSSeom_gyrBod(0,xModel(:,ki)',rFcur,Vl_ss,Vs_ss,h,Wi,l0+l_preload,m,K_ss,b_ss,gamx,gamy,rx,ry,alpha);
        ...
        case "lDSr"
            ...
            for ki = ki:k_end
                lFcur = lF-walkVel(1:2)*dt*(ki-k1);
                rFcur = rF-walkVel(1:2)*dt*(ki-k1);
                xModel(:,ki+1) = xModel(:,ki) + dt*lDSr_split_eom_gyrBod(0,xModel(:,ki)',lFcur,rFcur,Vl_ds,Vs_bl,Vs_fl,h,Wi,l0+l_preload,m,K_ds,0,gamx,gamy,rx,ry,alpha);
        ...
        case "rDSl"
            ...
            for ki = ki:k_end
                lFcur = lF-walkVel(1:2)*dt*(ki-k1);
                rFcur = rF-walkVel(1:2)*dt*(ki-k1);
                xModel(:,ki+1) = xModel(:,ki) + dt*rDSl_split_eom_gyrBod(0,xModel(:,ki)',lFcur,rFcur,Vl_ds,Vs_bl,Vs_fl,h,Wi,l0+l_preload,m,K_ds,0,gamx,gamy,rx,ry,alpha);
